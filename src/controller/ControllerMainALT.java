 ...
 */
public class ControllerMainALT implements ActionListener, ItemListener {
    ...
    Component leftStick,rightStick,leftBottomTrig, leftTopTrig, rightBottomTrig, rightTopTrig,
              dumpButton, minePosButton, raiseArmButton, scoopButton, povHat;    
    //Thread heartThread;
  ...
  public ControllerMainALT()
  {
    gui = new NewGuiALT(this);
    ...
    dumpButton = gamepad.pad.getComponent(Component.Identifier.Button._1);
    raiseArmButton = gamepad.pad.getComponent(Component.Identifier.Button._0);
    minePosButton = gamepad.pad.getComponent(Component.Identifier.Button._2);
    scoopButton = gamepad.pad.getComponent(Component.Identifier.Button._3);    
  ...
  public void altControllerCode()
  {
      ...
      float stickVal = leftStick.getPollData();
        ...
        if (changed == true)
        {
          ...
          if(lastLeftStick >= 0)
            data[0] = (byte) (LEFT_MASK | DIR_MASK | SPEED_MASK);
          else
            data[0] = (byte) (LEFT_MASK | SPEED_MASK);
          udp.send(data);
      ...
      stickVal = rightStick.getPollData();
        ...
        if (changed == true)
        {
          ...
          byte[] data = new byte[1];
          if(lastRightStick >= 0)
            data[0] = (byte) (RIGHT_MASK | DIR_MASK | SPEED_MASK);
          else
            data[0] = (byte) (RIGHT_MASK | SPEED_MASK);
          udp.send(data);
      ...
      if(buttonVal != lastRaiseArmButton)
      {
        ...
        {
          gui.addText("Face Button 1: Auto Raise Arm function");
          byte[] data = new byte[1];
          data[0] = AUTO_RAISE_ARM_MASK;
          udp.send(data);
      ...
      buttonVal = scoopButton.getPollData();
      ...
      if(buttonVal != lastScoopButton)
      {
        ...
        {
          gui.addText("Face Button 4: Auto Scoop function");
          byte[] data = new byte[1];
          data[0] = AUTOSCOOP_MASK;
          udp.send(data);
          lastScoopButton = 1;
        }        
      }
  ...
  {
    ControllerMainALT controller = new ControllerMainALT();
    controller.run();
  }
}
